...
Fast alle Tools sind read-only: kein Neustart von Services, keine destruktiven Kommandos (Whitelist + Blacklist in `server.py`), und das `plesk_api_get`-Tool kann nur GET-Requests stellen. Ausnahme: `write_vhost_file`, `delete_vhost_backup`, `dns_add_record` und `dns_delete_record` dürfen schreiben - die ersten beiden Dateien innerhalb von `/var/www/vhosts/<domain>/` anlegen/überschreiben/löschen (Backups), die letzten beiden DNS-Resource-Records der Domain-Zone anlegen/entfernen. Alle vier erfordern zwingend `confirm=true` pro Aufruf (keine globale Freischaltung), `write_vhost_file` legt vor dem Überschreiben automatisch ein Backup der alten Version an. Details siehe Abschnitt "Sicherheit" unten.
...
- `dns_add_record`/`dns_delete_record` erfordern beide `confirm=true` als expliziten Parameter pro Aufruf (kein globaler Schreib-Schalter) und rufen ausschliesslich `plesk bin dns --add`/`--del <domain> -<typ> ...` auf - unterstützte Record-Typen: A, AAAA, CNAME, MX, NS, TXT, SRV (PTR/CAA/DS/HTTPS/TLSA sind nicht abgedeckt, dafür weiterhin `plesk`-CLI direkt auf dem Server nutzen). Legt Plesk bereits einen identischen Record vor, meldet der Server selbst einen Fehler zurück (kein stillschweigendes Überschreiben/Duplizieren) - zum Ändern eines bestehenden Records daher erst `dns_delete_record`, dann `dns_add_record` mit dem neuen Wert. Beide Tools ändern live auflösbare DNS-Einträge auf einem Produktivserver.
